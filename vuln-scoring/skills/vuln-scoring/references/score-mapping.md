...
| 等级 | critical / high / medium / none（OH / Chrome / DB 官方低危另为 `low`） | Critical / High / Medium / Low / None |
...
| **0.1 – 3.9** | `none` 或 `medium`；OH / Chrome / DB 官方低危为 `low` | 有限影响；无安全意义则 none。**OpenHarmony / Chrome / DB 低危不要写成 `none`** |
| **0.0** | `none` | 无影响 |
