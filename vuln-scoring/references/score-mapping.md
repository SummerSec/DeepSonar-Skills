...
| 等级 | critical / high / medium / none（OH / Chrome / DB / Mobile 官方低危另为 `low`） | Critical / High / Medium / Low / None |
| 用途 | 是否报告、`severity_rule` | 向量、跨团队沟通、修复排序辅助 |
| 证据 | source→sink 或可复现利用 | 指标必须能被同一证据解释 |

> **官方四档域**（低危档 `low` 真实存在，不要写成 `none`）：`vuln-definitions-oh`、`vuln-definitions-chrome`、`vuln-definitions-db`、`vuln-definitions-mobile`。各域低危档定义见对应插件的 `references/severity-levels.md`。
...
| **0.1 – 3.9** | `none` 或 `medium`；OH / Chrome / DB / Mobile 官方低危为 `low` | 有限影响；无安全意义则 none。**OpenHarmony / Chrome / DB / Mobile 低危不要写成 `none`** |
| **0.0** | `none` | 无影响 |
