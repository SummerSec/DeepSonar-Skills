...
| ADJ2 | 仅非最新支持版本可复现 | 降档或排除（INV17 环境依赖 / INV22 非官方渠道视前提）；须在目标项目支持的最新版本确认 |
...
> 编号说明：INV1–INV14 通用排除、INV15–INV22 移动端领域排除、INV23–INV25 为目标项目（Google 与 Google 设备）专属，**INV26 回到通用条款（入口面本身不定漏洞），编号不连续是历史保留**——引用时以条款文字为准，不要按序号推断含义。写 finding 否决理由时引用条款文字，不引用序号。
...
| 「深链接路径遍历 = critical」 | 写 `.so` / 覆盖代码文件才 C1；仅写非执行文件 → H2（普通文件读写）；需多前提才可达 → H1 |
...
| 「用了 ECB / 固定 IV / 弱随机 = high」 | 算法弱本身不定档（单纯“弱”属理论，INV13）：须证明密文或密钥**可恢复且能接管**才 H4（AS5 / ID5），否则 M3 或排除 |
| 「申请了无障碍 / 通知监听权限 = 漏洞」 | 声明本身不是实害：须证明**未授权取得或滥用**后能读屏窃取凭据 / 自动化特权操作（AP6 → H9 / H4），否则 M9 或排除（INV13） |
