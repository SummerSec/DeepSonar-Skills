...
| 有限信息泄露 | M1 | M1 | M2 | 降档 |
| 仅资源耗尽 / 崩溃 / 本地杀进程 | 排除（INV1） | 排除（INV1） | 排除（INV1） | 排除 |
| 破坏性远程 DoS（恢复出厂级） | 排除（INV1） | 排除（INV1） | 排除（INV1） | 排除 |
...
| H8 | **已废止，不报。** 原「破坏性远程 DoS」并入 INV1（与本地 DoS / 纯崩溃同类）。编号保留以免旧 finding 锚点断裂；新报告不得填 `severity-levels.md#H8` |
