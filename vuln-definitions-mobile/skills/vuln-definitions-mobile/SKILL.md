...
description: "移动端（Android App + iOS App）领域漏洞定义指南。定义移动应用安全通用漏洞形态（Deep Link / URL Scheme 劫持与验证绕过、WebView XSS·RCE·JS bridge、Intent 重定向与劫持、组件导出、Content Provider 信息泄露·SQL 注入·路径遍历、广播劫持、数据存储与密钥、SSL/TLS 证书验证与固定绕过、认证与账户逻辑、内存安全），并把 HackerOne 移动端赏金惯例映射为 critical/high/medium/low 四档；含调整/无效条款、Gate 门禁、Android·iOS 攻击面索引与历史漏洞模式库（案例归纳 + 定级校准 + 挖掘切入点）。用户提到「移动端漏洞」「Android 漏洞」「iOS 漏洞」「APK」「IPA」「Deep Link」「URL Scheme」「WebView XSS」「addJavascriptInterface」「Intent 重定向」「组件导出」「Content Provider」「StrandHogg」「Task Hijacking」「证书固定绕过」「不安全数据存储」「移动端定级」或要给 Android / iOS 应用类审计目标定级时使用。This skill should be used when the user asks to rate a mobile app vulnerability (Android / iOS), classify deep link or URL scheme hijacking, WebView XSS or RCE, exported component or Content Provider issues, insecure data storage, certificate pinning bypass, or audits Android / iOS application targets."
---
...
- 涉及：Deep Link / App Links / intent scheme、URL Scheme / Universal Links、WebView 配置与 JS bridge、Intent 与组件导出、Content Provider、广播、文件与存储、密钥与凭据、SSL/TLS 与证书固定、OAuth 回调、认证与账户逻辑、任务 / 窗口
...
- **只挖**：已授权目标 App 的**应用层**组件与数据（APK / IPA 内逻辑、WebView、Deep Link、URL Scheme、导出组件、本地存储、网络栈）；目标项目的 HackerOne / 赏金页范围以项目页为准
- **报告**：官方四档 `critical` / `high` / `medium` / `low`
- **明确不报**：纯崩溃 DoS、self-XSS、理论无 PoC、静态分析器原始输出、依赖清单、缺限速 / 缺安全头、版本披露、需已越狱 / root 前提、仅非支持版本可复现、第三方库在出货路径不可达、系统层缺陷（走 `vuln-definitions-oh`）（见 `adjustment-and-invalid.md`）
...
4. mobile-vuln-types.md 定形态（Android AD/AW/AI/AE/AC/AB/AF/AA/AS/AT/AM；iOS IU/IO/IL/ID/IW/IM/IA）
...
11. 对照目标项目页填 bounty_eligible（赏金不改 severity）
...
mobile_class: <如 AD2|IU1|AW2>          # 移动端形态 ID，见 mobile-vuln-types.md
