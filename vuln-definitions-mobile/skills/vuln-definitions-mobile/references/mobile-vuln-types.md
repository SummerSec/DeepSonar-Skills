...
形态 ID 供 finding 的 `mobile_class` 字段使用。平台前缀：**AD**–AM 为 Android，**IU**–IA 为 iOS。
...
映射：`authz`（AE1/AE2）、`file-access`（AE4）。
...
| AA2 | 短信重发逻辑缺陷 / 限速 | Confused Deputy 绑定他人号码 + 全局限速 → 账户锁定 DoS（对照 Shopify #1416964） | M（DoS）/ H（若接管） |
...
| ID | 形态 | 典型机理 | 条款倾向 |
|----|------|----------|----------|
| IM1 | 内存消耗 DoS | 无限制分配 | 排除（DoS，见 `adjustment-and-invalid.md`） |
...
5. **系统层剥离**：内核 / 系统服务缺陷 → `vuln-definitions-oh`；应用层缺陷（WebView / Deep Link / 组件）→ 本插件
