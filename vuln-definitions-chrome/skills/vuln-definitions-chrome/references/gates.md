...
形态见 `chrome-vuln-types.md`；沙箱见 `process-sandbox.md`；范围见 `asset-scope.md`。
...
- [ ] **S5** 不是 VRP 明文排除且用户只关心赏金的配置（V8 `--experimental` 专属、SwiftShader、WebNN 未出货、`--single-process`、带警告的 unsafe 旗标）？仍要定级时可标 `security_impact: none`
...
- [ ] **R1** 内存安全：符号化 ASAN，含 MiraclePtr Status；`PROTECTED` → 停
- [ ] **R2** PoC 是附件文件（`poc.html` / `index.html`），不是外链站点
- [ ] **R3** 用 `chrome` 或 `d8`，不用测试二进制、不用 CDP/自定义 harness 包一层
- [ ] **R4** 已沦陷 renderer 用 MojoJS，而不是随意改 browser 补丁充数（补丁须解释为何 MojoJS 不够）
...
全过            → 四档定级，可写正式 finding
```

---

## 报告要求（对齐 VRP 习惯，供已授权投递）

1. **人工验证**；空描述 / 纯理论静态分析会被关
2. **版本与 OS**：Chrome/Chromium 版本、通道、OS/架构；鼓励 bisect
3. **最小化 PoC 文件**直接附上；不要 zip 打包一切；不要外链
4. **内存安全**：符号化 ASAN + 全部 additional information + MiraclePtr Status
5. **已沦陷 renderer**：优先 MojoJS；受控读写按上游 `vrp_flags` 方式证明（若走 VRP）
6. 不要完整武器化 exploit；验证路径与受控 r/w 证明即可
...
| “iframe sandbox 逃逸 = 出沙箱” | “源隔离问题，按 H9/H10，不是 C/H4” |
