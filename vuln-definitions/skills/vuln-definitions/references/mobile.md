...
| H8 | 破坏性远程 DoS：需恢复出厂设置、永久删除用户 / Profile 状态、无交互卸载 App，或反复呼出 / 阻止呼出紧急呼叫 |
...
| # | 条款 |
| --- | --- |
| INV1 | DoS / DDoS / 纯崩溃 / crash-only 内存破坏（**例外**：破坏性远程 DoS 按 `mobile.md#H8`） |
...
| 权限与访问控制 | 权限绕过 / WIU 保留、Special App Access、跨用户与 Private Space、破坏性远程 DoS、企业 DPC 绕过 | H8–H11 / M9 |
